...
const getPageContent = async (page, URL_INDEX) => {
    ...
    if (isLoggedIn === false) {
        ...
        await page.click("button.c-cUYkx.c-cUYkx-dshqME-variant-primary.c-cUYkx-fGHEql-isFullWidth-true.c-cUYkx-ABeol-size-large.hr-inline-flex.hr-justify-center.hr-align-center.hr-p-y-1");
        await page.waitForTimeout(2000);
    }
    const innerHTML = await page.$eval("div.challenge-body-html", (el) => el.innerHTML);
    ...
    const folderPath = path.join(process.cwd(), `playwright/exercises/week2/${exerciseTitle}`);
    try {
...
const getExercisesUploadFolder = async (page, browser) => {
    ...
    for (const anchor of anchors) {
        ...
        for (const span of spans) {
            ...
            if (textContent.trim() === "Solved" ||
                textContent.trim() === "Solve Challenge") {
                console.log('Found a "Solved" span within an anchor:', await anchor.getAttribute("href"));
